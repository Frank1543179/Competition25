...
def main():
    ...
    ap.add_argument("model_json", help="trained model JSON (Booster.save_model)") #训练好的模型JSON文件
    ap.add_argument("--test-csv", required=True, help="test CSV with header") #测试数据CSV文件
    ap.add_argument("--target", default=None, help="target column name (override model metadata)") #覆盖模型中的目标变量名
    ap.add_argument("--threshold", type=float, default=0.5, help="probability threshold for class 1 (default: 0.5)")#分类阈值，默认0.5
    args = ap.parse_args()
