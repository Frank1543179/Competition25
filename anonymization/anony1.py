...
def swap_age_groups(df: pd.DataFrame) -> None:
    ...
    # 随机选择10%的记录进行互换
    n_swap = min(int(len(young_indices) * 0.10), int(len(old_indices) * 0.10))
    ...
    n_swap2 = min(int(len(young_indices2) * 0.10), int(len(old_indices2) * 0.10))
...
def safe_ethnicity_perturbation(df: pd.DataFrame) -> None:
    ...
    # 将少量nonhispanic改为hispanic（5%），hispanic改为nonhispanic（3%）
    nonhispanic_mask = (df["ETHNICITY"] == "nonhispanic") & (np.random.rand(len(df)) < 0.05)
...
def micro_aggregate_medical_counts(df: pd.DataFrame) -> None:
    ...
    for col in medical_cols:
        ...
        for age_group in age_labels:
            ...
            # 添加基于年龄组分布的噪声，保持整数且在允许范围内
            noise = np.random.normal(0, std_val * 0.05, size=valid_mask.sum())
...
# === 5. 疾病标志位的条件扰动（调整depression_flag患病率）===
def conditional_flag_perturbation(df: pd.DataFrame) -> None:
    ...
    if old_stroke_mask.any():
        old_flip_indices = df[old_stroke_mask].sample(frac=0.02, random_state=42).index
        df.loc[old_flip_indices, "stroke_flag"] = "0"

    # 选择1%的低龄无中风改为有中风
    if young_no_stroke_mask.any():
        young_flip_indices = df[young_no_stroke_mask].sample(frac=0.01, random_state=42).index
    ...
    if middle_aged_obese.any():
        middle_flip_indices = df[middle_aged_obese].sample(frac=0.03, random_state=42).index
        df.loc[middle_flip_indices, "obesity_flag"] = "1"

    # 年轻有肥胖的4%改为无肥胖
    if young_obese.any():
        young_obese_flip = df[young_obese].sample(frac=0.04, random_state=42).index
        df.loc[young_obese_flip, "obesity_flag"] = "0"

    # asthma_flag轻微扰动
    if "asthma_flag" in df.columns:
        # 对所有记录的2%进行随机翻转
        flip_mask = (df["asthma_flag"].isin(["0", "1"])) & (np.random.rand(len(df)) < 0.02)
    ...
    if "depression_flag" in df.columns:
        ...
        target_depression_rate = 0.02  # 目标患病率9%
...
def smart_physiological_perturbation(df: pd.DataFrame) -> None:
    ...
    if "mean_systolic_bp" in df.columns:
        ...
        if valid_mask.any():
            ...
            # 对正常血压添加小噪声
            normal_noise = np.random.normal(0, 1.5, normal_bp_mask.sum())
            valid_bp.loc[normal_bp_mask] = (valid_bp[normal_bp_mask] + normal_noise).clip(90, 140)

            # 对异常血压适当向正常范围调整
            if abnormal_bp_mask.any():
                adjustment = np.random.normal(3, 2, abnormal_bp_mask.sum())
                # 高血压向下调，低血压向上调
    ...
    if "mean_weight" in df.columns:
        ...
        if weight_valid_mask.any():
            ...
            weight_noise = np.random.normal(0, 0.8, weight_valid_mask.sum())
    ...
    if "mean_bmi" in df.columns:
        ...
        if bmi_valid_mask.any():
            ...
            bmi_noise = np.random.normal(0, 0.3, bmi_valid_mask.sum())
