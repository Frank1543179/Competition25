...
def main():
    ...
    if "GENDER" in df.columns:
        df["GENDER"] = mutate_categorical(df["GENDER"], p=0.005)
    if "RACE" in df.columns:
        df["RACE"] = mutate_categorical(df["RACE"], p=0.015)
    if "ETHNICITY" in df.columns:
        df["ETHNICITY"] = mutate_categorical(df["ETHNICITY"], p=0.01)
    # ---- AGE ----
    process_age_add(df, col="AGE", lo=-1, hi=1, min_age=2, max_age=110)

    # ---- 整数列 ----
    process_int_column(df, "encounter_count",  lo=-5, hi=5)
    process_int_column(df, "num_procedures",   lo=-5, hi=5)
    process_int_column(df, "num_medications",  lo=-3,  hi=3)
