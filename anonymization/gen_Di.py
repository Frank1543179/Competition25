#用混合数据 Bi 与 Ci 学习模型 Di
#如果代码正确：89% → 隐私保护差
#如果代码有误：89% → 疾病预测准，但与隐私无关
...
#抽象类：初始化模型、保存 Di（训练好的模型）、对训练好的模型做 accuracy 评估等都在这个基类里实现。
#fit 方法是抽象的（没有实现），只是规定了接口（子类必须实现它）。
class DiGenBase(ABC):
...
#训练类：继承自 DiGenBase，实现了 fit：把分发的原始数据 Bi 和处理过的 Ci 混合（拼在一起）当作训练数据，并用 XGBoost 训练一个模型 Di。
...
if __name__ == "__main__":
    ...
    generator.save_Di("sample_D8.json")
    print("a Di.json example was saved as sample_D8.json")
