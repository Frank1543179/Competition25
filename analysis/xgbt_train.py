...
#二分类变量检查函数---严格检查Series是否为0/1二分类变量
...
def build_X(df: pd.DataFrame, target: str) -> pd.DataFrame:
    ...
    X_num_try = X_raw.apply(pd.to_numeric, errors="coerce") #数值型特征处理
    ...
    cat_cols = [c for c in X_raw.columns if c not in num_cols]#分类型特征处理
...
def main():
    ...
    ap.add_argument("train_csv", help="training CSV with header") #训练数据CSV文件路径
    ap.add_argument("--model-json", required=True, help="output model JSON path")#输出模型JSON文件路径
    ap.add_argument("--target", default="stroke_flag", help="binary target column (default: stroke_flag)")#目标变量名，默认"stroke_flag"
    ap.add_argument("--seed", type=int, default=42, help="random seed (default: 42)")
    ap.add_argument("--test-size", type=float, default=0.1, help="validation ratio (default: 0.1)")#验证集比例，默认0.1
    ap.add_argument("--n-estimators", type=int, default=600, help="n_estimators (default: 600)")#树的数量，默认600
    ap.add_argument("--max-depth", type=int, default=6, help="max_depth (default: 6)")# 树的最大深度，默认6
    ap.add_argument("--learning-rate", type=float, default=0.05, help="learning_rate (default: 0.05)")# 学习率，默认0.05
    ap.add_argument("--subsample", type=float, default=0.9, help="subsample (default: 0.9)")#样本采样比例，默认0.9
    ap.add_argument("--colsample-bytree", type=float, default=0.9, help="colsample_bytree (default: 0.9)")#特征采样比例，默认0.9
    ap.add_argument("--early-stopping-rounds", type=int, default=50, help="early stopping rounds (default: 50)")#早停轮数，默认50
    args = ap.parse_args()
    ...
    model = xgb.XGBClassifier(
        ...
        tree_method="hist", #使用直方图算法提高训练速度
        n_estimators=args.n_estimators,
        max_depth=args.max_depth,
        learning_rate=args.learning_rate,
        subsample=args.subsample,
        colsample_bytree=args.colsample_bytree,
        early_stopping_rounds=args.early_stopping_rounds,  # ← 警告回避。在构造函数中设置早停，避免警告
    ...
    #模型评估
    y_pred = (model.predict_proba(X_val)[:, 1] >= 0.5).astype(int)#使用0.5作为分类阈值预测验证集,计算准确率并输出
